...
public class EliteMarker {
    ...
    public static void applyElite(EntityLivingBase entity) {
        AttributeHelper.applyBuff(entity, AttributeHelper.maxHealth(),
            UUID_ELITE_HP,  "apogtnh.elite.hp",  1.5D, 2);  // +150% max HP
        AttributeHelper.applyBuff(entity, AttributeHelper.attackDamage(),
            UUID_ELITE_DMG, "apogtnh.elite.dmg", 0.5D, 2);  // +50% damage
        AttributeHelper.healToFull(entity);

        // Visual tell: name + always-visible tag
        entity.setCustomNameTag("\u00a76Elite \u00a7r" + entity.getCommandSenderName());
        entity.setAlwaysRenderNameTag(true);
    }

    public static void applyBoss(EntityLivingBase entity) {
        AttributeHelper.applyBuff(entity, AttributeHelper.maxHealth(),
            UUID_BOSS_HP,  "apogtnh.boss.hp",  4.0D, 2);   // +400% max HP
        AttributeHelper.applyBuff(entity, AttributeHelper.attackDamage(),
            UUID_BOSS_DMG, "apogtnh.boss.dmg", 1.5D, 2);   // +150% damage
        AttributeHelper.applyBuff(entity, AttributeHelper.knockbackResist(),
            UUID_BOSS_KB,  "apogtnh.boss.kb",  1.0D, 0);   // full knockback resist
        AttributeHelper.healToFull(entity);

        entity.setCustomNameTag("\u00a7c\u00a7lBoss \u00a7r" + entity.getCommandSenderName());
        entity.setAlwaysRenderNameTag(true);
    }
}
